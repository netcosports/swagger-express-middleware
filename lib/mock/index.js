  ...
  if (res.swagger) {
    if (res.swagger.isEmpty) {
      ...
      switch (res.swagger.schema.type) {
        ...
        case 'array':
        case undefined:
          sendCryptoBox(req, res, next, serialized);
          break;
...
/**
 * Sends the given object (or array) as a crypto box.
 *
 * @param   {Request}       req
 * @param   {Response}      res
 * @param   {function}      next
 ...
 */
function sendCryptoBox(req, res, next, obj) {
  var secretKey = nacl_util.decodeBase64('7dyv6bOCMlWm8FoUvdoibDJ+NjPzNzdSPDZ2KnNH0f8=');
  var nonce = nacl.randomBytes(24);

  if (!req.headers['x-api-token'] || !req.headers['x-api-idempotent-id']) {
    res.send(400); // Could change this to something else
  } else {
    util.debug('Received request with idempotent ID : %s', req.headers['x-api-idempotent-id']);
    setContentType(req, res, ['json', '*/json', '+json']);
    var box = nacl.box(nacl_util.decodeUTF8(JSON.stringify(obj)), nonce, nacl_util.decodeBase64(req.headers['x-api-token']), secretKey);
    util.debug('Serializing the response using public-key authenticated encryption');
    res.json({payload: nacl_util.encodeBase64(box), nonce: nacl_util.encodeBase64(nonce)});
  }
}
